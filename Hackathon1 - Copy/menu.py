...
class Menu:
    def display_menu(self, active_user):
        ...
        add_button = tk.Button(text="Add New Favorite", command=lambda: self.return_func('a'))
        search_button = tk.Button(text="Search Recipe Database", command=lambda: self.return_func('s'))
        view_button = tk.Button(text="View Favorites", command=lambda: self.return_func('v'))
