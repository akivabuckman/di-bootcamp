...
def xp3():
    ...
    print(
        f"each dictionary only has one item per key. in the previous step we made number of stores 10,000, which replaced"
...
def xp4():
    ...
    disney_users_A = {item: index for index, item in enumerate(users)}
    print(f'A: {disney_users_A}')

    disney_users_B = {index: item for index, item in enumerate(users)}
    print(f"B: {disney_users_B}")

    disney_users_C = {item: index for index, item in enumerate(sorted(users))}
    print(f"C: {disney_users_C}")

    # Only recreate the 1st result for:
    # The characters, which names contain the letter “i”.
    disney_users_D = {item: index for index, item in enumerate(users) if 'i' in item}
    print(f'D: {disney_users_D}')

    # Only recreate the 1st result for:
    # The characters, which names start with the letter “m” or “p”.
    disney_users_E = {item: index for index, item in enumerate(users) if item[0].lower() in "mp"}
    print(f'E: {disney_users_E}')
